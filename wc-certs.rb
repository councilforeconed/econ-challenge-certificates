...
CSV.foreach('./wc-input.csv', headers: true) do |row|
  teacher = row[0]
  name = "#{row[1]} #{row[2]}"
  division = row[3]
  ...
  Prawn::Document.generate "output/#{teacher}/#{filename}.pdf", page_layout: :landscape do
    ...
    text "National Economics Challenge", size: 24, align: :center
    move_down 10
    text "2016 Wild Card Competition", size: 24, align: :center
    move_down 15
    font "./resources/OpenSans-Regular.ttf"
