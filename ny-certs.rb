...
CSV.foreach('./ny-input.csv', headers: true) do |row|
  school = row[0].titleize
  name = "#{row[1]} #{row[2]}"
  division = row[3]
  foldername = school.parameterize
  filename = name.parameterize
  Dir.mkdir("./output/ny/#{foldername}") unless File.exists?("./output/ny/#{foldername}")
  Prawn::Document.generate "output/ny/#{foldername}/#{filename}.pdf", page_layout: :landscape do
    ...
    text "2017 New York State Finals", size: 24, align: :center
    move_down 15
    font "./resources/OpenSans-Regular.ttf"
